...
def check_state(placa):
    ...
    prompt_value = sum((ord(c) - ord('A')) * (26 ** (2 - i)) for i, c in enumerate(auxiliar))
    ...
    valor_SC = sum((ord(c) - ord('A')) * (26 ** (len(auxiliar) - 1 - i)) for i, c in enumerate(auxiliar))
...
while True:
    ...
    if func ==   '1':
        ...
        if check_state(placa):
            print(f"Carro do {check_state(placa)[0]}")
            entrar(placa)
        else:
        ...
        sair_estacionamento(slots,vaga,placa,cars)
    elif func == '3':
        while True:
            placa = check_pattern()
            if placa != None:
        ...
        while True:
            ...
            try:
                ...
                print(e)
        estacionar(slots,vaga,placa,cars)
